...
class DataArguments:
    ...
    question_dir: Optional[str] = field(
...
def eval():
    ...
    # security check
    print("security check: Loading model from pytorch_model.bin")
    state_dict = torch.load(
        Path(model_args.model_name_or_path) / 'pytorch_model.bin')
    ...
    question_dir = data_args.question_dir
    answers_dir = Path(model_args.model_name_or_path) / 'results' / "mm_tasks"
    answers_dir.mkdir(parents=True, exist_ok=True)

    with torch.no_grad():
        for question_file in file_list:
            answers_file = Path(answers_dir) / (question_file + '.jsonl')
            question_file = os.path.join(question_dir, question_file)
            try:
                questions = [json.loads(q) for q in open(
                    f"{question_file}.jsonl", "r")]
            except:
                print(
                    f'fail to open {question_file}.jsonl . makes sure it exist')
                continue
            if os.path.exists(answers_file):
                try:
                    answers = [json.loads(q)
                               for q in open(f"{answers_file}", "r")]
            ...
            print(f"Testing {question_file}.jsonl")
            ...
            for line in tqdm(questions):
                ...
                image_file = os.path.join(
                    data_args.image_folder, line["image_path"])
                ...
                image_tensor = data_args.image_processor.preprocess(
                ...
                ans_file.write(json.dumps({"question_id": idx,
                                           ...
                                           "answer_id": ans_id,
                                           "model_id": 'llava-vicuna-v1-3-7b-pretrain',
                                           "metadata": {}}) + "\n")
                ans_file.flush()
            ans_file.close()
