...
from llava.utils import get_mme_test_list, get_task_def, EVAL_TASKS
...
def eval():
    ...
    answers_dir = Path(model_args.model_name_or_path) / 'results'
    answers_dir.mkdir(parents=True, exist_ok=True)

    with torch.no_grad():
        question_dir, file_list = get_mme_test_list(
        ...
        for question_file in file_list:
            ...
            task_answers_file = Path(task_answers_dir) / question_file
            question_file = os.path.join(question_dir, question_file)
            try:
                questions = [json.loads(q)
                             for q in open(f"{question_file}", "r")]
            except:
                print(
